...
if (( apass > 0 ))
then
  rrpct=`echo "scale=2; $apass / $max * 50.0" | bc -l`
fi
...
if (( gpass > 0 ))
then
  #gpct=`echo "scale=2; ($gpass / $max * 90.0) + $ec" | bc -l`
  gpct=`echo "scale=2; $rrpct + $nppct + $ppct + $ec" | bc -l`
fi

echo ""
printf "%20s:   - / -   %6.1f%% of 90%%\n" "Tests" $gpct
